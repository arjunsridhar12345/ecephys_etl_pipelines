...
def get_stim_epoch_vsync(sync: Dataset):
    """
...
def generate_behavior_stim_table(
    ...
    stim_df = get_stimulus_presentations(behavior_pkl.data, epoch_timestamps, frame_offset=frame_offset)
    ...
    stim_df['start_frame'] = [start_frame + frame_offset for start_frame in stim_df.start_frame.values]
    stim_df['end_frame'] = [end_frame if np.isnan(end_frame) else end_frame + frame_offset for end_frame in stim_df.end_frame.values]
...
def set_corrected_times(
    ...
    if vsync_slice.stop < len(corrected_frame_times):
        ...
        if len(start_time + corrected_relevant_vsyncs[
            0:
...
def get_stim_order(sync: Dataset, pkl_file_list: list) -> np.array:
    ...
    print('Num pkl frames', number_pkl_frames)
    sync_stim_epoch_frames = get_stim_epoch_vsync(sync)
    print('Sync stim epoch', sync_stim_epoch_frames)
...
def create_dynamic_gating_stimulus_table(
    sync_dataset: Dataset,
    behavior_pkl: BehaviorPickleFile,
    mapping_pkl: CamStimOnePickleStimFile,
    ...
    # determine order of stimuli 
    order = get_stim_order(sync_dataset, [behavior_pkl, mapping_pkl])

    if order[0] == 0:
        frame_counts = [
            pkl.num_frames for pkl in (behavior_pkl, mapping_pkl)
        ]
    else:
        frame_counts = [
            pkl.num_frames for pkl in (mapping_pkl, behavior_pkl)
        ]
    ...
    if order[0] == 0:
        behavior_offset = frame_offsets[0]
        mapping_offset = frame_offsets[1]

        stim_behavior_start = stim_starts[0]
        stim_behavior_end = stim_ends[0]

        stim_mapping_start = stim_starts[1]
        stim_mapping_end = stim_ends[1]
    else:
        behavior_offset = frame_offsets[1]
        mapping_offset = frame_offsets[0]

        stim_behavior_start = stim_starts[1]
        stim_behavior_end = stim_ends[1]

        stim_mapping_start = stim_starts[0]
        stim_mapping_end = stim_ends[0]
    ...
    behavior_df = generate_behavior_stim_table(
        ...
        stim_behavior_start,
        stim_behavior_end,
        frame_offset=behavior_offset,
        coming_after_mapping=True
    )
    """
    ...
    if order[0] == 0:
        full_stim_df = pd.concat([behavior_df, mapping_df], sort=False)
    else:
        full_stim_df = pd.concat([mapping_df, behavior_df], sort=False)
